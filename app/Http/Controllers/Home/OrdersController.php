...
class OrdersController extends Controller
{
    ...
    //交易大厅
    public function currency_trade(Request $request)
    {
    ...
    public function getOrders(Request $request)
    {
        switch (I('post.type')) {
            case 'buy':
                $this->ajaxReturn($this->getOrdersByType(I('post.currency_id'), 'buy', 10, 'desc'));
                break;
            case 'sell':
                $this->ajaxReturn($this->getOrdersByType(I('post.currency_id'), 'sell', 10, 'asc'));
                break;
        }
    }
    //获取k线
    public function getOrdersKline(Request $request)
    {
        if (empty($_GET['currency'])) {
            return;
        }
