...
//         $my_invit = M('Member')->field('email,status,reg_time')->where(array('pid'=>session('USER_KEY_ID')))->select();
     ...
     * 系统消息
     */
    public function sysMassage(Request $request)
    {
        $member_id = session('USER_KEY_ID');
        $M_member = M('Message');
     ...
     *显示详细系统消息界面
     */
    public function showSysTem(Request $request)
    {
    ...
    public function chexiaoByid(Request $request)
    {
        $withdraw = M('Withdraw');
        $member = M('Member');
        $id = I("post.id");
...
//           $this->ajaxReturn($op);
     ...
     * ajax上传图片方法
     */
    function addPicForAjax(Request $request)
    {
        //头像上传
        $upload = new Upload();// 实例化上传类
        $upload->maxSize   =     3145728 ;// 设置附件上传大小
...
//            $pic=ltrim($pic_1,".");
    ...
    *  我的众筹显示页面
    */
    public function zhongchou(Request $request)
    {
    ...
    public function substitution_center(Request $request)
    {
